...
impl Scanner {
    ...
    pub fn pop(&mut self) -> Option<&char> {
        self.characters.get(self.cursor).map(|char| {
            self.cursor += 1;
            char
        })
    }

    /// Returns true if the `target` is found at the current cursor position, and advances the cursor.
    /// Otherwise, returns false leaving the cursor unchanged.
    pub fn take(&mut self, target: &char) -> bool {
        if self.characters.get(self.cursor) == Some(&target) {
            self.cursor += 1;
            true
        } else {
            false
    ...
    /// Iteratively directs the advancement of the cursor and the return of translated values.
    pub fn scan<T>(&mut self, cb: impl Fn(&str) -> Option<Action<T>>) -> Result<Option<T>, Error> {
        ...
        loop {
            match self.characters.get(self.cursor) {
                ...
                None => {
                    break if require { Err(Error::EndOfLine) } else { Ok(request) };
                }
            }
        }
    }
    ...
    pub fn transform<T>(&mut self, cb: impl FnOnce(&char) -> Option<T>) -> Option<T> {
        self.characters.get(self.cursor).and_then(|char| {
            cb(char).map(|output| {
                self.cursor += 1;
                output
            })
