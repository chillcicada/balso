use balso_core::{AtomKind, Bracket, Element, Selection, Shortcut, Symbol};
...
impl Atom {
    ...
    pub fn valence(&self) -> u8 {
        ...
        let bond_order_sum = self.bonds.iter().fold(0, |s, b| s + b.kind.bond_order());
    ...
    pub fn implicit_hydrogens(&self) -> u8 {
        match &self.kind {
            ...
            AtomKind::Selection(_) => self.subvalence().saturating_sub(1),
    ...
    pub fn is_heteroatom(&self) -> bool {
        let element: Element = match &self.kind {
            AtomKind::Star => return false,
            AtomKind::Shortcut(shortcut) => shortcut.into(),
            AtomKind::Selection(selection) => selection.into(),
        ...
        !matches!(element, Element::C | Element::H)
    }
}

#[cfg(test)]
mod valence {
    use balso_core::{Element, Symbol, VirtualHydrogen};
...
#[cfg(test)]
mod subvalence {
    use balso_core::{Element, Selection, Symbol, VirtualHydrogen};
    use pretty_assertions::assert_eq;
...
mod implicit_hydrogens {
    use balso_core::{Bracket, Element, Selection, Shortcut, Symbol};
    use pretty_assertions::assert_eq;
    ...
    fn shortcut_carbon_neighbors_elided() {
        let atom = Atom::shortcut(Shortcut::C, vec![Bond::elided(1)]);
    ...
    fn selection_carbon_neighbors_elided() {
        let atom = Atom::selection(Selection::C, vec![Bond::elided(1)]);
...
#[cfg(test)]
mod hydrogens {
    use balso_core::{Bracket, Element, Selection, Shortcut, Symbol, VirtualHydrogen};
    use pretty_assertions::assert_eq;
...
mod is_heteroatom {
    ...
    fn star() {
        ...
        assert_eq!(atom.is_heteroatom(), false)
    }
    ...
    #[test]
    fn shortcut_oxygen() {
        let atom = Atom::shortcut(Shortcut::O, vec![]);

        assert_eq!(atom.is_heteroatom(), true)
    }
    ...
    #[test]
    fn selection_oxygen() {
        let atom = Atom::selection(Selection::O, vec![]);
    ...
    fn bracket_oxygen() {
        let atom = Atom::bracket(
            Bracket {
                symbol: Symbol::Element(Element::O),
                ..Default::default()
            },
            vec![],
        );
    ...
    fn bracket_selection() {
        let atom = Atom::bracket(
            Bracket {
                symbol: Symbol::Selection(Selection::N),
                ..Default::default()
            },
            vec![],
        );
