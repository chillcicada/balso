use balso_core::{AtomKind, BondKind, Bracket, Selection, Shortcut};
...
mod valence {
    use super::*;
    use balso_core::VirtualHydrogen;
...
mod implicit_hydrogens {
    ...
    fn shortcut_carbon_input_elided_children_none() {
        let atom = Atom::shortcut(balso_core::Shortcut::C, vec![]);
        let input = Some(&balso_core::BondKind::Elided);
    ...
    fn selection_carbon_input_elided_children_none() {
        let atom = Atom::selection(balso_core::Selection::C, vec![]);
    ...
    fn bracket_carbon_input_elided_children_none() {
        let atom = Atom::bracket(
            balso_core::Bracket {
                symbol: balso_core::Symbol::Element(balso_core::Element::C),
                ..Default::default()
            },
            vec![],
        );
...
mod hydrogens {
    ...
    fn bracket_hydrogens_some() {
        let atom = Atom::bracket(
            balso_core::Bracket {
                hydrogens: Some(balso_core::VirtualHydrogen::H2),
                ..Default::default()
            },
            vec![],
        );
